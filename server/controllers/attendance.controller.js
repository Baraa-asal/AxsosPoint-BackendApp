    ...
    .then(async (res) => {
      const labeledDesc = res.map((photo, key) => {
        if (photo?.desc?.length && photo?.desc?.length > 0) {
          return new faceapi.LabeledFaceDescriptors(photo._id.toString(), [
            Float32Array.from(Object.values(JSON.parse(photo?.desc))),
          ]);
        }
      });
      faceMatcher = new faceapi.FaceMatcher(labeledDesc, 0.6);
      console.log(faceMatcher);
      let detectionFace = await faceMatcher.findBestMatch(
        Object.values(req.body.desc)
      );
      ...
      .then((photo) => {
        ...
        }).catch(err=> console.log("Unknow Person"));
